...
func NewDockerv2(w http.ResponseWriter, r *http.Request, rec record.Record, c config.Config) *Dockerv2 {
	return &Dockerv2{
		rw:  w,
		req: r,
		rec: rec,
...
func (d *Dockerv2) Redirect() error {
	...
	if !strings.HasPrefix(path, "/v2") {
		...
		if path == "" || path == "/" {
			record.Fallback(d.rw, d.req, "root", http.StatusPermanentRedirect, config.Config{})
			return nil
		}
		record.Fallback(d.rw, d.req, "website", http.StatusPermanentRedirect, config.Config{})
		return nil
	}
	...
	if path != "/" {
		...
		d.rw.Header().Add("Cache-Control", fmt.Sprintf("max-age=%d", variables.Status301CacheAge))
