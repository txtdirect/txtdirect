...
func NewPath(w http.ResponseWriter, r *http.Request, path string, rec record.Record, c config.Config) *Path {
	return &Path{
		Rw:   w,
		Req:  r,
		Path: path,
...
func (p *Path) Redirect() *record.Record {
	...
	*p.Req = *rec.AddToContext(p.Req)
	if err != nil {
		...
		record.Fallback(p.Rw, p.Req, "to", p.Rec.Code, p.C)
		return nil
	}
...
func (p *Path) RedirectRoot() error {
	...
	if p.Rec.Code == http.StatusMovedPermanently {
		p.Rw.Header().Add("Cache-Control", fmt.Sprintf("max-age=%d", variables.Status301CacheAge))
	}
...
func (p *Path) SpecificRecord() (*record.Record, error) {
...
func (p *Path) specificMatch(regexes RegexRecords) (*record.Record, error) {
	...
	*p.Req = *p.Req.WithContext(context.WithValue(p.Req.Context(), "regexMatches", recordMatch[keys[len(keys)-1]].Submatches))
	...
	var err error
	if rec, err = record.ParseRecord(recordMatch[keys[len(keys)-1]].TXT, p.Rw, p.Req, p.C); err != nil {
...
func (p *Path) fetchRegexes() (RegexRecords, error) {
	...
	for i, loop := 1, true; loop != false; i++ {
		txts, err := query.Query(fmt.Sprintf("%d.%s", i, p.Req.Host), p.Req.Context(), p.C)
...
// the given record.
func zoneFromPath(r *http.Request, rec record.Record) (string, int, []string, error) {
	path := r.URL.Path
	...
	if rec.Re != "" {
		...
		if GroupRegex.MatchString(rec.Re) {
			...
			*r = *r.WithContext(context.WithValue(r.Context(), "regexMatches", unordered))
			...
			url = append([]string{variables.Basezone}, url...)
...
func getFinalRecord(zone string, from int, c config.Config, w http.ResponseWriter, r *http.Request, pathSlice []string) (record.Record, error) {
	txts, err := query.Query(zone, r.Context(), c)
	if err != nil {
	...
	if err != nil || len(txts) == 0 {
		return record.Record{}, fmt.Errorf("could not get TXT record: %s", err)
	}

	txts[0], err = record.ParsePlaceholders(txts[0], r, pathSlice)
	var rec record.Record
	if rec, err = record.ParseRecord(txts[0], w, r, c); err != nil {
		return rec, fmt.Errorf("could not parse record: %s", err)
	}

	if rec.Type == "path" {
		records := r.Context().Value("records").([]record.Record)
