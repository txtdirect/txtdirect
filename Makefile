...
docker-test:
	docker run -v $(shell pwd):/go/src/github.com/txtdirect/txtdirect golang:1.12rc1-alpine /bin/sh -c "cd /go/src/github.com/txtdirect/txtdirect && apk add git gcc musl-dev make && GOROOT=\"/usr/local/go\" make test"

docker-build:
	docker run -v $(shell pwd):/go/src/github.com/txtdirect/txtdirect golang:1.12rc1-alpine /bin/sh -c "cd /go/src/github.com/txtdirect/txtdirect && apk add git gcc musl-dev make && make build && rm -rf caddy-copy"
